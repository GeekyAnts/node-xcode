...
pbxProject.prototype.addPbxGroup = function(filePathsArray, name, path, sourceTree, customFramework, targetName) {
    ...
    for (var index = 0; index < filePathsArray.length; index++) {
        ...
        var file = new pbxFile(filePath, {customFramework: customFramework});
        file.uuid = this.generateUuid();
        file.fileRef = this.generateUuid();
        this.addToPbxFileReferenceSection(file);    // PBXFileReference
        this.addToPbxBuildFileSection(file);        // PBXBuildFile
        this.addToPbxFrameworksBuildPhaseCustom(file, targetName);
...
pbxProject.prototype.addToPbxFrameworksBuildPhaseCustom = function(file, targetName) {
    var targetKey = this.findTargetKey(targetName);
    var sources = this.pbxFrameworksBuildPhaseObj(targetKey);
    sources.files.push(pbxBuildPhaseObj(file));
}
...
pbxProject.prototype.addBuildPropertyCustom = function(prop, value, build_name) {
    var configurations = nonComments(this.pbxXCBuildConfigurationSection()),
        key, configuration;

    for (key in configurations){
        configuration = configurations[key];
        if ((!build_name || configuration.name === build_name) && configuration.buildSettings && configuration.buildSettings.CURRENT_PROJECT_VERSION){
            configuration.buildSettings[prop] = value;
        }
    }
}
